...
best_student = df1.loc[df1['Average'].idxmax()]
...
df2['Product_A_Percentage_Change'] = df2['Product_A'].pct_change() * 100
df2['Product_B_Percentage_Change'] = df2['Product_B'].pct_change() * 100
df2['Product_C_Percentage_Change'] = df2['Product_C'].pct_change() * 100
...
avg_salary_department = df3.groupby('Department')['Salary'].mean()
...
df4['Product'].value_counts().plot(kind='pie', autopct='%1.1f%%', title='Sales Distribution by Product')
plt.show()
...
solutions = [
    ...
    df1, best_student, 
    # Sales Data
    total_sales, highest_sales_date, 
    # Employee Information
    avg_salary_department, most_experienced, 
    # Customer Orders
    total_revenue, most_ordered_product
]
